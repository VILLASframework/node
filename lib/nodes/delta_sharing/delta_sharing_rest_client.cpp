...
DeltaSharingRestClient::ListSchemas(const DeltaSharingProtocol::Share &share,
  ...
  for (auto it = items.begin(); it < items.end(); it++) {
    DeltaSharingProtocol::Schema s =
        it.value().get<DeltaSharingProtocol::Schema>();
    p->push_back(s);
  }
  return p;
};
...
DeltaSharingRestClient::ListTables(const DeltaSharingProtocol::Schema &schema,
  ...
  c->SetUserAgent(this->user_agent);
  std::string path =
      "/shares/" + schema.share + "/schemas/" + schema.name + "/tables";
  RestClient::Response r = c->get(path);
  json j = json::parse(r.body);
  auto items = j["items"];
...
const DeltaSharingProtocol::Metadata DeltaSharingRestClient::QueryTableMetadata(
    const DeltaSharingProtocol::Table &table) const {
  std::unique_ptr<RestClient::Connection> c =
      std::unique_ptr<RestClient::Connection>(
          new RestClient::Connection(this->profile.endpoint));
  c->SetUserAgent(this->user_agent);
  std::string path = "/shares/" + table.share + "/schemas/" + table.schema +
                     "/tables/" + table.name + "/metadata";
  RestClient::Response r = c->get(path);
...
const DeltaSharingProtocol::DeltaSharingProfile &
DeltaSharingRestClient::GetProfile() const {
  return this->profile;
}
...
void DeltaSharingRestClient::PopulateCache(const std::string &url,
  ...
  if (!std::filesystem::exists(completePath + "/" + path)) {
    std::cerr << completePath + "/" + path << " does not exist in cache"
              << std::endl;
    ...
    while (this->shouldRetry(r)) {
      ...
      if (cnt > 4) {
        std::cerr << "Failed to retrieve file using url: ( Response code: "
                  << r.code << ") Message: " << r.body << std::endl;
        return;
      }
      r = this->get(url);
    }
    ...
    if (r.code != 200) {
      std::cerr << "Could not read file: " << r.code << " Message: " << r.body
                << std::endl;
      return;
    }

    std::fstream f;
    f.open(completePath + "/" + path,
