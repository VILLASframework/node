...
static std::string
    shmemAsyncName; // Shared Memory identifiers and size, provided via argv or config.
static std::string
...
static std::vector<unsigned> idsSend,
    idsRecv; // A list of available send / receive icon IDs.
static spdlog::sink_ptr sink;

static Opal_GenAsyncParam_Ctrl
    params; // String and Float parameters, provided by the RT-LAB OpAsyncGenCtrl block.
