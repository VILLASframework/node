...
public:
  ...
  int parse(json_t *json) override {
    ...
    try {
      server = config.at("server").get<std::string>();
      protocol = config.at("protocol").get<std::string>();
      client_id = config.value("client_id", client_id);

      if (auto out = config.find("out"); out != config.end())
        produce = out->value("topic", produce);

      if (auto in = config.find("in"); in != config.end()) {
        consume = in->value("topic", consume);
        group_id = in->value("group_id", group_id);
      }

      if (auto to = config.value("timeout", -1.0); to >= 0)
        timeout = std::chrono::milliseconds(static_cast<int>(to * 1000));

      if (auto ssl = config.find("ssl"); ssl != config.end())
        ssl_ca = ssl->value("ca", ssl_ca);

      if (auto sasl_cfg = config.find("sasl"); sasl_cfg != config.end()) {
        sasl.mechanisms = sasl_cfg->at("mechanisms").get<std::string>();
        sasl.username = sasl_cfg->at("username").get<std::string>();
        sasl.password = sasl_cfg->at("password").get<std::string>();
      }
