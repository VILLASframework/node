...
public:
  ReflectionClient(std::shared_ptr<grpc::Channel> channel) : stub_(grpc::reflection::v1alpha::ServerReflection::NewStub(channel)) {}
  google::protobuf::FileDescriptorSet *GetFileDescriptor(const std::string& symbol) {
    ...
    google::protobuf::FileDescriptorSet *file_descs = new google::protobuf::FileDescriptorSet;
    if (stream->Read(&response)) {
        if (response.has_file_descriptor_response()) {
          for (const auto& bytes : response.file_descriptor_response().file_descriptor_proto()) {
            google::protobuf::FileDescriptorProto *file_desc = file_descs->add_file();
...
public:
  ...
  Response *executeGet() {
    ...
    auto call = generic_stub.PrepareUnaryCall(&context, methodFullName, req_buf, &cq);
    grpc::Status status;
    call->StartCall();
    ...
    if (!status.ok()) {
      cq.Shutdown();
      throw Error(HTTP_STATUS_NOT_FOUND, nullptr, "Cannot connect to gRPC server");
    }
  ...
  Response *executePost() {
    ...
    FileDescriptorSet *protos = refl_client.GetFileDescriptor(gRPC_package + "." + gRPC_service);
      ...
      if (const FieldDescriptor* f = req_desc->field(i)) {
        ...
        if (json_val->type == json_type::JSON_ARRAY) {
          if (f->is_repeated()) {
            ...
            for (size_t j = 0; j < ele_count; j++) {
              ...
              switch (f->type()) {
                case grpc::protobuf::FieldDescriptor::TYPE_STRING:
                  refl->AddString(request.get(), f, json_string_value(json_array));
                  break;
                case grpc::protobuf::FieldDescriptor::TYPE_INT32:
                  refl->AddInt32(request.get(), f, json_integer_value(json_array));
                  break;
                case grpc::protobuf::FieldDescriptor::TYPE_INT64:
                  refl->AddInt64(request.get(), f, json_integer_value(json_array));
                  break;
            ...
            std::cerr << "Json array in non repeat field not support" << std::endl;
            continue;
          }
        } else {
          switch (f->type()) {
  ...
  grpc::ByteBuffer form_request() {
    ...
    if (body) {
      ...
      double timestamp = 0;
      json_t *json_value;
      ret = json_unpack_ex(body, &err, 0, "{ s: F, s: o}",
      ...
      sample_dummy->flags = (int)SampleFlags::HAS_DATA | (int)SampleFlags::HAS_TS_ORIGIN;
      ...
      size_t dummy_wbytes;
      ret = gateway_node->formatter->sprint(dummy_buf, dummy_buflen, &dummy_wbytes, sample_dummy);
      if (ret < 0) {
  ...
  json_t *parse_response_reflection(const google::protobuf::Descriptor *resp_desc, std::string &recv_data) {
    google::protobuf::DynamicMessageFactory dym_factory;
    json_t *root = json_object();
    const google::protobuf::Message* resp_prototype = dym_factory.GetPrototype(resp_desc);
          ...
          if (f->is_repeated()) {
            ...
            for (int j = 0; j < resp_refl->FieldSize(*response, f); j++) {
              ...
              case grpc::protobuf::FieldDescriptor::Type::TYPE_STRING:
                json_array_append(json_arr, json_string(resp_refl->GetRepeatedString(*response, f, j).c_str()));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_INT32:
                json_array_append(json_arr, json_integer(resp_refl->GetRepeatedInt32(*response, f, j)));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_INT64:
                json_array_append(json_arr, json_integer(resp_refl->GetRepeatedInt64(*response, f, j)));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_DOUBLE:
                json_array_append(json_arr, json_real(resp_refl->GetRepeatedDouble(*response, f, j)));
                break;
                case grpc::protobuf::FieldDescriptor::Type::TYPE_BOOL:
                json_array_append(json_arr, json_boolean(resp_refl->GetRepeatedBool(*response, f, j)));
                break;
              default:
                break;
              }
            ...
            switch (f->type()) {
              case grpc::protobuf::FieldDescriptor::Type::TYPE_STRING:
                json_object_set_new(root, f->name().c_str(), json_string(resp_refl->GetString(*response, f).c_str()));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_INT32:
                json_object_set_new(root, f->name().c_str(), json_integer(resp_refl->GetInt32(*response, f)));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_INT64:
                json_object_set_new(root, f->name().c_str(), json_integer(resp_refl->GetInt64(*response, f)));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_DOUBLE:
                json_object_set_new(root, f->name().c_str(), json_real(resp_refl->GetDouble(*response, f)));
                break;
              case grpc::protobuf::FieldDescriptor::Type::TYPE_BOOL:
                json_object_set_new(root, f->name().c_str(), json_boolean(resp_refl->GetBool(*response, f)));
                break;
              default:
                break;
            }
  ...
  json_t *parse_response(grpc::ByteBuffer &resp_buf, std::string &recv_data) {
    ...
    Sample *sample_dummy = sample_alloc_mem(64);
    int ret = gateway_node->formatter->sscan(recv_data.c_str(), resp_buf.Length(), &rbytes, sample_dummy);
    if (ret < 0) {
      std::cerr << "Formatting Failed: " << ret << std::endl;
    }
    for (unsigned int i = 0; i < sample_dummy->length; i++) {
      json_array_append(json_ch, sample_dummy->data[i].toJson(SignalType::FLOAT));
    }
    json_sigs = json_pack_ex(&err, 0, "{s: f, s: o}",
                  "timestamp",  time_to_double(&sample_dummy->ts.origin),
                  "value", json_ch);
...
static char r[] = "/gateway/(" RE_NODE_NAME ")/([A-Za-z0-9_-]+)/([A-Za-z0-9_-]+)/([A-Za-z0-9_-]+)";
