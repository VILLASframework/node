...
Test(timing, time_now) {
  struct timespec const now1 = time_now();
  struct timespec const now2 = time_now();

  cr_assert(time_cmp(&now1, &now2) <= 0, "time_now() was reordered!");
}

Test(timing, time_diff) {
  struct timespec const ts1 = {.tv_sec = 0,
                               .tv_nsec = 1}; // Value doesnt matter
  struct timespec const ts2 = {.tv_sec = 1,
                               .tv_nsec = 0}; // Overflow in nano seconds!

  struct timespec const ts3 = time_diff(&ts1, &ts2);
...
Test(timing, time_add) {
  struct timespec const ts1 = {.tv_sec = 1,
                               .tv_nsec = 999999999}; // Value doesnt matter
  struct timespec const ts2 = {.tv_sec = 1,
                               .tv_nsec = 1}; // Overflow in nano seconds!

  struct timespec const ts3 = time_add(&ts1, &ts2);
...
Test(timing, time_delta) {
  struct timespec const ts1 = {.tv_sec = 1,
                               .tv_nsec = 123}; // Value doesnt matter
  struct timespec const ts2 = {.tv_sec = 5,
                               .tv_nsec = 246}; // Overflow in nano seconds!

  double const delta = time_delta(&ts1, &ts2);

  cr_assert_float_eq(delta, 4 + 123e-9, 1e-9);
}

Test(timing, time_from_double) {
  double const ref = 1234.56789;

  struct timespec const ts = time_from_double(ref);
...
Test(timing, time_to_from_double) {
  ...
  struct timespec const ts = time_from_double(ref);
  double const dbl = time_to_double(&ts);
