...
void to_json(villas::Json &json, ::json_t const *jansson) {
  ...
  case JSON_OBJECT: {
    ...
    //
    // There is no API in libjansson to iterate or even enumerate keys of a const object.
    //
    // See https://github.com/akheron/jansson/issues/578
    json_object_foreach (const_cast<::json_t *>(jansson), key, value)
...
void from_json(Json const &json, JanssonPtr &jansson) {
  ...
  case value_t::object: {
    ...
    for (auto const &[key, value] : json.items())
      json_object_set_new_nocheck(jansson.get(), key.c_str(),
                                  value.get<JanssonPtr>().release());
  } break;

  case value_t::string: {
    auto const string = json.get<Json::string_t const *>();
    jansson.reset(json_string_nocheck(string->c_str()));
  } break;
...
#else
  auto logger = Log::get("config");
  logger->warn("Loading configuration using an outdated version of libconfig");
  logger->warn(
      "@include directives may not work as expected and can't be disabled");
  if (include_dir)
    ::config_set_include_dir(&config, include_dir->c_str());
#endif
...
#else
    throw std::runtime_error(
        "VILLASnode was built without support for libconfig");
#endif // WITH_CONFIG
  } else {
