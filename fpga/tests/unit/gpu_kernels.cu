...
__global__ void gpu_rtds_rtt_kernel(
    volatile uint32_t *dataIn, volatile reg_doorbell_t *doorbellIn,
    volatile uint32_t *dataOut,
    volatile villas::fpga::ip::ControlRegister *controlRegister, int *run) {
  ...
  while (*run) {
    ...
    while (not(doorbellIn->is_valid and (last_seq != doorbellIn->seq_nr)) and
           *run)
      ;
...
void gpu_rtds_rtt_start(
    volatile uint32_t *dataIn, volatile reg_doorbell_t *doorbellIn,
    volatile uint32_t *dataOut,
    volatile villas::fpga::ip::ControlRegister *controlRegister) {
  printf("run:         %p\n", run);
  if (run == nullptr) {
  ...
  *run = 1;
  gpu_rtds_rtt_kernel<<<1, 1>>>(dataIn, doorbellIn, dataOut, controlRegister,
                                run);
