...
should_build() {
	...
	*)
	...
	if { [[ "${DEPS_SKIP:-}" == *"${id}"* ]] || { [[ -n "${DEPS_INCLUDE+x}" ]] && [[ ! "${DEPS_INCLUDE}" == *"${id}"* ]]; }; }; then
...
# Build and install nlohmann_json for node delta-sharing
if ! pkg-config "nlohmann_json" &&
	should_build "nlohmann_json" "for the delta-sharing node-type"; then
